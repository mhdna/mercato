    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <template v-else>
          ...
          <v-divider class="my-4" />
          <div class="text-subtitle-2 mb-2">Recurring targets</div>
          <v-alert v-if="!seriesList.length" class="mb-4" type="info" variant="tonal">
            No recurring targets set up -- each period (e.g. every month) creates its own
            target automatically, starting fresh at $0.
          </v-alert>
          <v-table v-else class="mb-4" density="compact">
            <thead>
              <tr><th /><th>Schedule</th><th>Target</th><th>Active</th></tr>
            </thead>
            <tbody>
              <tr v-for="s in seriesList" :key="s.id">
                <td><span class="target-color-swatch" :style="{ background: s.color }" /></td>
                <td>{{ describeSeries(s) }}</td>
                <td>${{ (s.target_amount / 100).toLocaleString() }}</td>
                <td>
                  <v-switch
                    color="primary"
                    density="compact"
          ...
          <form class="field-grid" @submit.prevent="submitSeries">
            ...
            <div class="d-flex justify-end field-full">
              <v-btn color="primary" :loading="seriesSaving" text="Add Recurring Target" type="submit" />
            </div>
          </form>
        </template>
      </v-card-text>
...
<script setup>
  ...
  async function submitSeries () {
    ...
    targetsError.value = ''
    try {
      await createBranchTargetSeries(targetsBranch.value.id, {
