<template>
  <div class="page-root">
    ...
    <v-card class="dl-card" flat>
      ...
      <div class="dl-layout">
        <div class="dl-sidebar">
          ...
          >
            <v-list-item
              v-for="dl in discountLists"
              :key="dl.id"
              :value="dl.id"
            >
              <v-list-item-title>{{ dl.name }}</v-list-item-title>
              <template #append>
                <v-chip v-if="dl.is_default" color="primary" size="x-small" text="Default" />
                <v-icon
                  v-else-if="!dl.is_active"
                  color="grey"
        ...
        <div class="dl-content">
          <div v-if="!selectedList" class="pa-6 text-medium-emphasis">
            Select a discount list on the left, or add one.
          </div>
          ...
          <template v-else>
            ...
            <div class="d-flex align-end ga-3 pa-4">
              ...
              <v-btn
                color="primary"
                :disabled="!addProductId || addDiscount === ''"
                :loading="addingItem"
                text="Add"
                variant="flat"
                @click="addItem"
            ...
            >
              <template #item.discount="{ item }">{{ item.discount }}%</template>
    ...
    <v-dialog v-model="listDialog" max-width="520">
      <v-card>
        <v-card-title>{{ editingList ? 'Edit Discount List' : 'Add Discount List' }}</v-card-title>
        <v-card-text>
    ...
    <v-dialog v-model="deleteListDialog" max-width="420">
      <v-card>
        <v-card-title>Delete discount list</v-card-title>
        <v-card-text>
          Delete <strong>{{ selectedList?.name }}</strong> and all its discounts? This can't be undone.
        </v-card-text>
        <v-card-actions>
          <v-spacer />
    ...
    <BranchAssignDialog
      ...
      kind="discount list"
      :load-fn="() => fetchDiscountListBranchIds(selectedList.id)"
      :save-fn="ids => setDiscountListBranches(selectedList.id, ids)"
...
<script setup lang="ts">
  ...
  const loadError = ref('')
  const selection = ref([])
  const selectedList = computed(() => discountLists.value.find(d => d.id === selection.value[0]) ?? null)

  const itemHeaders = [
    { title: 'Code', key: 'product_code', align: 'start', width: 140 },
    { title: 'Product', key: 'product_name', align: 'start' },
    { title: 'Discount', key: 'discount', align: 'end', width: 140 },
  ...
  async function loadItems () {
    ...
    itemError.value = ''
    try {
      items.value = await fetchDiscountListItems(selectedList.value.id)
    } catch (error) {
  ...
  onMounted(async () => {
    try {
      ...
      if (selection.value.length === 0 && discountLists.value.length > 0) {
        selection.value = [discountLists.value[0].id]
      }
    } catch (error) {
  ...
  async function addItem () {
    if (!addProductId.value || addDiscount.value === '') return
    const pct = clampPct(addDiscount.value)
    if (pct === null) {
      itemError.value = 'Enter a discount between 0 and 100.'
      return
    }
    addingItem.value = true
    itemError.value = ''
    try {
      await saveDiscountListItem({ discountListId: selectedList.value.id, productId: addProductId.value, discount: pct })
  ...
  const editItemDiscount = ref('')
  const savingItem = ref(false)

  function openEditItem (item) {
    editItemTarget.value = item
    editItemDiscount.value = String(item.discount)
    editItemDialog.value = true
  }

  async function saveEditItem () {
    const pct = clampPct(editItemDiscount.value)
    if (pct === null) return
    savingItem.value = true
    try {
      await saveDiscountListItem({ discountListId: selectedList.value.id, productId: editItemTarget.value.product_id, discount: pct })
  ...
  async function removeItem (item) {
    try {
      await deleteDiscountListItem(selectedList.value.id, item.product_id)
      await loadItems()
    } catch (error) {
      itemError.value = error.message
    }
  ...
  async function saveList () {
    ...
    try {
      if (editingList.value) {
        await updateDiscountList({ id: selectedList.value.id, ...payload })
      } else {
        const created = await createDiscountList(payload)
  ...
  async function doDeleteList () {
    ...
    try {
      await deleteDiscountList(selectedList.value.id)
      deleteListDialog.value = false
      selection.value = discountLists.value.length > 0 ? [discountLists.value[0].id] : []
    } catch (error) {
      loadError.value = error.message
    } finally {
